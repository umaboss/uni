...
const ArticleList = () => {
  ...
  const togglePopular = (id) => {
    const updated = articles.map((a) => a.id === id ? { ...a, popular: !a.popular } : a );
    setArticles(updated);
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                <tr><td colSpan={7} className="text-center py-12 text-gray-500 text-lg">Loading...</td></tr>
              ) : articles.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-12 text-gray-500 text-lg">No articles found.</td></tr>
              ) : (
                articles.map((article) => {
                  // Prepend backend URL if the image path is relative
                  const imageUrl = article.image && article.image.startsWith('/uploads') ? `http://localhost:5000${article.image}` : article.image || "/assets/placeholder.svg";
                  return (
                    <tr key={article.id} className="hover:bg-gray-50">
                      <td className="py-3 px-4">
                        <img src={imageUrl} alt={article.title} className="w-16 h-16 object-cover rounded" />
                      </td>
                      ...
                      <td className="py-3 px-4 text-center">
                        <span className={article.is_active ? "text-green-600 font-semibold" : "text-red-600 font-semibold"}>
                          {article.is_active ? "Yes" : "No"}
                        </span>
                      </td>
                      <td className="py-3 px-4 text-gray-600">{article.createdAt ? new Date(article.createdAt).toLocaleString() : "-"}</td>
                      <td className="py-3 px-4 text-center">
